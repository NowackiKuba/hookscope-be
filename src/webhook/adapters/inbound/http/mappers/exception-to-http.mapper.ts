...
import {
  ...
  WebhookAlertNotFoundException,
} from '@webhook/domain/exceptions';
...
export class ExceptionToHttpMapper extends BaseExceptionToHttpMapper {
  static map(exception: DomainException, path?: string): HttpExceptionResponse {
    ...
    if (exception instanceof WebhookAlertNotFoundException) {
    ...
    if (exception instanceof UnknownProviderException) {
      return this.createResponse(
        exception,
  ...
  static getStatusCode(exception: DomainException): HttpStatus {
    if (
      exception instanceof WebhookAlertNotFoundException ||
